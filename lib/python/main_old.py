...
import visibility
# Tools
from tools import dbg, get_ip_addresses, setHostNPort
...
host, port = None, None
visibility.initDiscoverySocket()
...
while (True):
    ...
    if (host is not None):
        visibility.makeServerVisible(host)
    ...
    if (host is not None):
        ...
        while (True):
            try:
                if (host is not None):
                    clientAddr = visibility.acceptConnection(host)
